...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        #eval a call expression
        operator = scheme_eval(first, env)
        def build_operands_helper(rest):
            if rest is nil:
                return nil
            return Pair(scheme_eval(rest.first, env), build_operands_helper(rest.rest))
        operands = build_operands_helper(rest)
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        try:
            # BEGIN PROBLEM 2
            "*** YOUR CODE HERE ***"
            return procedure.py_func(*li) # unpack the list into a bunch of args
            # END PROBLEM 2
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        new_frame = procedure.env.make_child_frame(procedure.formals, args) #env is where the lambda is being called, not defined
