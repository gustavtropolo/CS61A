...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        #this is a variable for a value, not a procedure
        var_name = expressions.first
        bound_value = scheme_eval(expressions.rest.first, env)
        env.define(var_name, bound_value)
        return var_name
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    if curr is nil:
        return True
    while curr.rest is not nil and is_scheme_true(scheme_eval(curr.first, env)):
        curr = curr.rest
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    # expressions is Pair(10, Pair(Pair('print', Pair(1, nil)), nil))
    curr = expressions
    if curr is nil:
        return False
    while curr.rest is not nil and is_scheme_false(scheme_eval(curr.first, env)):
        curr = curr.rest
    return scheme_eval(curr.first, env)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    names = vals = nil
    # BEGIN PROBLEM 14
    # Pair('let', Pair(Pair(Pair('x', Pair(5, nil)), nil), Pair(Pair('+', Pair('x', Pair(3, nil))), nil)))
    "*** YOUR CODE HERE ***"
    binding = bindings
    while binding is not nil:
        try:
            name, val = binding.first.first, scheme_eval(binding.first.rest.first, env)
        except Exception:
            raise SchemeError('Wrong argument format')
        if not (isinstance(name, str)):
            raise SchemeError
        if duplicate_name(name, names):
            raise SchemeError(f"duplicate name: '{name}'")
        validate_form(binding.first.rest, 1, 1) #1 is the max number of args
        names, vals = Pair(name, names), Pair(val, vals)
        binding = binding.rest
    # we need to reach in and grab the elements and turn them into lists
