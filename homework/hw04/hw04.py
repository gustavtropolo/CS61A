def shuffle(s):
    ...
    "*** YOUR CODE HERE ***"
    length = len(s)
    firstHalfIt = iter(s[0:length // 2])
    secondHalfIt = iter(s[length // 2:])
    return [next(firstHalfIt) if i % 2 == 0 else next(secondHalfIt) for i in range(length)]
...
def balanced(m):
    ...
    if is_planet(m):
        return True
    leftT = length(left(m)) * total_mass(end(left(m)))
    rightT = length(right(m)) * total_mass(end(right(m)))
    return leftT == rightT and balanced(end(left(m))) and balanced(end(right(m)))
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        return True
    for b in branches(t):
        if berry_finder(b): #instead of returning individual result, we return true if any of the branches
            return True
    return False
...
def max_path_sum(t):
    ...
    maxBranch = 0
    for b in branches(t):
        path = max_path_sum(b) #must try each of the paths to see which is longest
        if path > maxBranch:
            maxBranch = path
