...
def floor_div(args):
    ...
    while curr is not nil:
        if isinstance(curr.first, Pair): # we have a subexpression to evaluate
            num = num // calc_eval(curr.first)
        else:
            num = num // curr.first
        curr = curr.rest
    return num
...
def eval_and(expressions):
    ...
    curr = expressions
    while curr.first is not scheme_f and curr.rest is not nil:
        curr = curr.rest
    if isinstance(curr.first, Pair):
        return calc_eval(curr.first)
    return curr.first #curr.first is the first val that fails, or the last val
...
def eval_define(expressions):
    ...
    if isinstance(expressions.rest.first, Pair):
        res = calc_eval(expressions.rest.first)
    else:
        res = expressions.rest.first
        if res in bindings:
            res = bindings[res]
        if res in OPERATORS:
            res = OPERATORS[res]
