...
def buy(fruits_to_buy, prices, total_amount):
    ...
    [9 apples][1 kiwi]
    """
    def add(fruits, amount, cart): #recurse until we fill cart
        if fruits == [] and amount == 0: #we bought ever type of fruit and have no money left
            print(cart)
        elif fruits and amount > 0: #the fruits left to buy is still not empty
            fruit = fruits[0] 
            price = prices[fruit] #get the price of the fruit considered
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    ax, ay = get_lon(city_a), get_lat(city_a)
    bx, by = get_lon(city_b), get_lat(city_b)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    distanceA = distance(city_a, location)
    distanceB = distance(city_b, location)
